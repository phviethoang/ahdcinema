...
const AddShowtimes = () => {
  ...
  // const movies = [
  //   { id: 1, name: "Spider-Man: No Way Home", duration: 180 },
  //   { id: 2, name: "Black Panther", duration: 121 },
  //   { id: 3, name: "Doctor Strange in the Multiverse of Madness", duration: 125 },
  //   { id: 4, name: "Avengers: Infinity War", duration: 180 },
  // ];
...
//   const itemsPerPage = 5; // Số lượng suất chiếu mỗi trang

//   const paginationState = Object.fromEntries(
//     cinemas.map(cinema => [cinema.id, { currentPage: 1, itemsPerPage: 5 }])
//   );
...
//         return a.roomId - b.roomId; // roomId nhỏ hơn lên trước
//       }
//       return calculateDateTime(a.date, a.time) - calculateDateTime(b.date, b.time);
//     });

// const currentShowtimes = sortedShowtimes.slice(startIndex, startIndex + itemsPerPage);
...
const itemsPerPage = 5; // Số suất chiếu mỗi trang
...
const handlePrevPage = (cinemaId) => {
  ...
  const handleAddShowtime = () => {
    ...
    // const cinema = cinemas.find((cinema) => cinema.id === parseInt(selectedCinema));
    console.log(room);
    const conflict = isTimeConflict(parseInt(selectedRoom), selectedDate, selectedTime, movie.duration);
    if (conflict) {
      const movieConflict = movieData.find(movie=>movie.id===conflict.movieId);
      setNotification({
  ...
  const handleSaveEdit = () => {
    ...
    // const startDateTime = new Date(`${editingShowtime.show_date}T${editingShowtime.show_time }`);
    // const endDateTime = new Date(startDateTime);
  ...
  // const totalPages = Math.ceil((groupedShowtimes[selectedCinema]?.length || 0) / itemsPerPage);
  const totalPages = selectedCinema
  ? Math.ceil((groupedShowtimes[selectedCinema]?.length || 0) / itemsPerPage)
  : 0; // Nếu selectedCinema không có giá trị hợp lệ thì totalPages sẽ là 0
          ...
          <div className={styles.modalContent}>
            <h3>Chỉnh sửa suất chiếu</h3>
              <div className={styles.formRow}>
                ...
                {/* <select
                  name="movieName"
                  // value={editingShowtime.movieName}
                  value={movieData.find(movie=>movie.movie_id===editingShowtime.movie_id).movie_name}

                  onChange={(e) => setEditingShowtime({ ...editingShowtime, editingShowtime.movie_id: e.target.value })}
                >
                  {movies.map((movie) => (
